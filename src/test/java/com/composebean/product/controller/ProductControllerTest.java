...
class ProductControllerTest {
    ...
    void getProducts() throws Exception {
        ...
        when(productService.getProducts(null))
                .thenReturn(new ProductListResponse(List.of(product)));
        ...
        mockMvc.perform(get("/api/products"))
                ...
                .andExpect(jsonPath("$.products[0].name")
                        .value("콜롬비아 원두"))
                .andExpect(jsonPath("$.products[0].price").value(18000))
                .andExpect(jsonPath("$.products[0].stockQuantity")
                        .value(100));
    ...
    void getProduct() throws Exception {
        ...
        mockMvc.perform(get("/api/products/{productId}", 1L))
                ...
                .andExpect(jsonPath("$.name").value("콜롬비아 원두"))
                .andExpect(jsonPath("$.price").value(18000))
                .andExpect(jsonPath("$.description")
                        .value("산미와 단맛이 균형 잡힌 원두"))
    ...
    void createProductValidationFail() throws Exception {
        ...
        mockMvc.perform(post("/api/products")
                ...
                .andExpect(jsonPath("$.errors.price")
                        .value("상품 가격은 0원 이상이어야 합니다."))
                .andExpect(jsonPath("$.errors.stockQuantity")
                        .value("재고 수량은 0개 이상이어야 합니다."));
    }
    ...
    void updateProduct() throws Exception {
        ProductResponse response = ProductResponse.builder()
                ...
                .imageUrl(
                        "https://example.com/images/ethiopia-beans.jpg"
                )
                .stockQuantity(100)
        ...
        mockMvc.perform(put("/api/products/{productId}", 1L)
                ...
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.name").value("에티오피아 원두"))
    ...
    void updateStock() throws Exception {
        ProductResponse response = ProductResponse.builder()
                ...
                .description("산미와 단맛이 균형 잡힌 원두")
                .imageUrl(
                        "https://example.com/images/colombia-beans.jpg"
                )
                .stockQuantity(75)
                .createdAt(LocalDateTime.of(
                        2026, 7, 20, 15, 44, 42
    ...
    @Test
    @DisplayName("JSON 필드 타입이 올바르지 않으면 400을 반환한다")
    void invalidJsonType() throws Exception {
        String request = """
                {
                  "name": "콜롬비아 원두",
                  "price": "비쌈",
